...
class ViewController: UIViewController {
  ...
  // VOLUME
  private var startObjectDetection : Bool = true
  ...
  func drawAfterPerformingCalculations(
    ...
    for detection in detections {
      ...
      if convertedRect.maxX > self.overlayView.bounds.maxX {
        ...
        let center = 6 * convertedRect.origin.x + 3 * convertedRect.size.width;
        let imageWidth = self.overlayView.bounds.maxX;

        var location = 0
        if (center > 4 * imageWidth) {
            location =  2;
        } else if (center < 2 * imageWidth) {
            location =  1;
        } else {
            location =  0;
        }
...
struct ConstantsDefault {
  ...
  static let theadCountLimit = 1 //10
}
